<?php
	...
	$singleGuide = '<li class="js-single"><a href="%s" title="Learn more about %s"%s aria-label="%s" tabindex="0">%s</a></li>';

	$dropdownGuide = '<li><a href="%s" title="Learn more about %s"%s aria-label="%s" tabindex="-1">%s</a></li>';
	...
	foreach($res as $r){
		...
		if(count($items) == 0){	// this is a single item link
			...
			$return .= '<li id="dropdown-'.$r->post_name.'" title="Learn more about '.$r->post_title.'" class="js-dropdown" aria-label="Learn more about '.$r->post_title.'"><a href="javascript:void();" title="'.$r->post_title.' dropdown menu" aria-label="'.$r->post_title.' dropdown menu">'.$r->post_title.'</a><ul role="menu">';
