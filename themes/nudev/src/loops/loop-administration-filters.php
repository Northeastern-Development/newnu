	...
	if($field){
		foreach($field['choices'] as $k => $v){
			if(!in_array($v,$skipThese)){
				$return .= '<li><a '.($filter == strtolower(str_replace(" ","-",$v))?'class="active"':'').' href="'.home_url().'/about/university-administration/'.strtolower(str_replace(" ","-",$v)).'" title="Show '.strtolower($v).' team">'.$v.'</a></li>';
			}
		}
	}
